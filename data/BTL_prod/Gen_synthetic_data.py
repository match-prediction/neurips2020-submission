...
def Gen_data_under_BTL(GroundTruth, p_obs, l_rep, M):
	...
	for i in range(num_dataset):
		...
		group_A_score = np.prod(GroundTruth[group[:M]])
		group_B_score = np.prod(GroundTruth[group[M:]])
...
def main(n_items, score_range, p_obs, l_rep, M):
	...
	np.savetxt(os.path.join('data/BTL_prod', 'BTL_prod.GT'), GroundTruth)
	np.savetxt(os.path.join('data/BTL_prod', 'BTL_prod.edges'), pairwise_data)
	np.savetxt(os.path.join('data/BTL_prod', 'BTL_prod.edges_test'), pairwise_data_test)
	np.savetxt(os.path.join('data/BTL_prod', 'BTL_prod.nodes'), feature)
